...
<<<<<<< HEAD
const friendships = () => knex.select('*').from('friendships')
  .then((allFriendships) => allFriendships);

const friendList = (_, { id }) => knex.from('friendships').where('user1_id', id).orWhere('user2_id', id)
  ...
  .then((userFriendships) => {
    const userFriendDetails = [];
    userFriendships.map((friend) => {
      if (friend.user1_id.toString() !== id.toString()) {
        userFriendDetails.push({
          friend: {
            id: friend.user1_id,
...
export const deleteFriendship = async (_, { id }) => knex('friendships')
  .where({ id }).del().then((result) => result);
=======
