...
describe('Test PodcastGraphQL User resolvers with mock-knex', () => {
	describe('Get should return', () => {
		it('all podcasts', () => {
			return podcastGraphql.default.podcasts()
				.then((podcasts) => {
		...
		it('the podcast with the matching id', () => {
			const findId = 2;
			return podcastGraphql.default.podcast(_, { id: findId })
				.then((podcast) => {
	...
	describe('Create should', () => {
		describe('fail to create podcast ', () => {
			mochaEach([
				...
				.it('with Missing Parameters: %j', (newPodcast) => {
					return podcastGraphql.createPodcast(_, { input: newPodcast })
						.then((podcast) => {
